...
import { messageToastAtom } from "../../state";

export function MessageToast() {
  const [message, setMessage] = useAtom(messageToastAtom);
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-white rounded-full shadow-lg border border-gray-200 pl-5 pr-5 py-1.5">
      <span className="text-sm text-gray-700 whitespace-nowrap truncate max-w-[50vw]">{snippet}</span>
    </div>
  );
}
